...
if __name__ == '__main__':
    ...
    model.predict(X)
    #tp = np.logical_and(y, pr).sum()
    # fp = np.logical_and(1 - y, test_preds).sum()
    # tn = np.logical_and(1 - y, 1 - test_preds).sum()
    # fn = np.logical_and(y, 1 - test_preds).sum()
    #
    # # calculate binary classification metrics
    # recall = tp / (tp + fn)
    # precision = tp / (tp + fp)
    # accuracy = (tp + tn) / (tp + fp + tn + fn)
    # print(pd.crosstab(y, test_preds, rownames=['actuals'],
    #                   colnames=['predictions']))
    # print("\n{:<11} {:.3f}".format('Recall:', recall))
    # print("{:<11} {:.3f}".format('Precision:', precision))
    # print("{:<11} {:.3f}".format('Accuracy:', accuracy))
